...
class DropUpload extends React.Component {
  state = {
    upload: [],
  ...
  handleUploadError = () => {
    Push.create('Загрузка файлов', {
      body: 'Файл не был загружен',
      icon: '/img/error.jpg',
      timeout: 6000,
    });
  };
  ...
  handleFileInput = (e, files) => {
    ...
    const matchesReqs = file.size <= 20 * 1024 * 1024 && /png|jpg|jpeg|pdf/i.test(file.type);
    if (!matchesReqs) {
      this.handleAferUpload();
      return;
    }

    const upload = Documents.insert(insertObject);
    ...
    return (
      ...
      >
        ...
        <label className="clickableDrop__button" id="fakeUpload" htmlFor="userfile">
          ...
          <h2 className="clickableDrop__header">Upload media</h2>
        </label>}
