...
if (Meteor.isServer) {
  ...
  Documents.on('afterUpload', function (fileRef) {
  ...
  Documents.interceptDownload = (http, fileRef, version) => {
    ...
    if (pipePathExists) {
      ...
      s3.getObject(opts, function (error) {
        ...
        Documents.serve(http, fileRef, fileRef.versions[version], version, dataStream);
  ...
  const _origRemove = Documents.remove;

  Documents.remove = (search) => {
    const cursor = Documents.collection.find(search);
    ...
    _origRemove.call(Documents, search);
  };

  Documents.denyClient();

  // PUBLICATIONS
  Meteor.publish('getDocumentsOfUser', userId => Documents.find({ userId }).cursor);
}
...
Documents.collection.attachSchema(modifiedSchema);
